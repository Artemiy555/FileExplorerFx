...
public class ClassTreeView extends ExplorerFx {
    ClassTreeView() {
    }
        ...
        for (int i = 0; i < fl.length; i++) {
            ...
            if (!fl[i].isFile() && !fl[i].isHidden() && fl[i].isDirectory() && n == 0) {
                ...
                try {
                    ...
                    System.out.println("Проходим " + fl[i].getAbsolutePath() + " " + x.getMessage());
                                    }
            }
        }
    ...
    public void CreateTreeView(TreeView<String> treeview) {
        ...
        for (int i = 0; i < sysroots.length; i++) {
            ...
            try {
                ...
                System.out.println("Проходим по: " + x.fillInStackTrace() + drives[i].toString());
            } finally {
